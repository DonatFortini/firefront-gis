...
use firefront_gis_lib::fetch_resources;

#[tokio::test]
async fn test_fetch_forest_shp_url_valid() {
    let url = fetch_resources::get_departement_shp_file_url(
        "2A",
...
async fn test_fetch_forest_shp_url_invalid() {
    let error = fetch_resources::get_departement_shp_file_url(
        "99",
        "https://geoservices.ign.fr/bdforet#telechargementv2",
    )
    .await
...
async fn test_download_forest_shp() {
    ...
    fetch_resources::download_shp_file(url, "2A").await.unwrap();
